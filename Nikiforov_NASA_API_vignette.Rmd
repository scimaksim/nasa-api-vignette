...
The `annualExoDiscoveries()` function retrieves the latest data from NASA's Exoplanet Archive. 
...
```
As of `r date()`, the archive's [Planetary Systems Composite Parameters](https://exoplanetarchive.ipac.caltech.edu/docs/API_PS_columns.html) (PSCompPars) table lists `r length(exoplanetData$pl_name)` confirmed exoplanet observations. By default, the function targets the Planetary Systems Composite Parameters Table and excludes bodies which have had their planetary status "questioned in the published literature" (@https://doi.org/10.26133/nea13). Moreover, to refrain from complex orbital mechanics, the function also excludes circumbinary systems - ones in which planets orbit two stars.  

We can compare the composition of extrasolar systems against that of our own, with its one star and eight planets. 
