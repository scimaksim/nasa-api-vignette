...
exoData %>% mutate(innerHZ = NA, outerHZ = NA)
exoData %>% mutate(spectralClass = NA)
...
habitableExoFinder <- function(data, minEarthMass = 0.1, maxEarthMass = 10, 
                               minEarthRadius = 0.5, maxEarthRadius = 2.5){
  
  habitablePlanets <- data %>% select(pl_name, spectralType, pl_bmasse, pl_rade) %>% 
    filter(spectralType %in% c("F", "G", "K", "M") & (pl_bmasse >= minEarthMass) &
...
habitableExoPlanets <- habitableExoFinder(exoData)
