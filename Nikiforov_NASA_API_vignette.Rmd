...
The `annualExoDiscoveries()` function retrieves the latest data from NASA's Exoplanet Archive. By default, the function targets the Planetary Systems Composite Parameters Table and excludes bodies which have had their planetary status "questioned in the published literature" (@https://doi.org/10.26133/nea13). Moreover, to refrain from complex orbital mechanics, the function also excludes circumbinary planets - systems in which planets orbit two stars.  
...
As of `r date()`, the NASA Exoplanet Archive's [Planetary Systems Composite Parameters](https://exoplanetarchive.ipac.caltech.edu/docs/API_PS_columns.html) (PSCompPars) table lists `r length(exoplanetCount$pl_name)` confirmed exoplanet observations. 

We can compare the composition of these systems against that of our own solar system, with its one star and eight planets. 
...
`r starPlanetFreq[1, 1]` systems - more than `r round((starPlanetFreq[1, 1])/(length(exoplanetCount$pl_name))*100, 1)`% of all observed - orbit a single star and have no other planetary companions. Another `r starPlanetFreq[1, 2]` systems (`r round((starPlanetFreq[1, 2])/(length(exoplanetCount$pl_name))*100, 1)`%) have one star and two planets. `r starPlanetFreq[3, 5]` systems are particularly interesting, with three stars and five planets in their system. One observations contains a planet with a record four stars. 

We can also enumerate the annual number of exoplanet findings since 1989 and identify 2014 and 2016 as the most prolific years for discovery.  
